...
import { setProductFilter } from "../../redux/slice/ProductFilterSlice.jsx";
...
const Header = () => {
  ...
  const [navNameToId, setNavNameToId] = useState({});
  ...
  const handleNavigationClick = (item, idx) => {
    ...
    if (route) {
      ...
      setOpenMobileMenu(null);
      setIsMobileMenuOpen(false);
      // If this route is the products listing, push navigationID into Redux
      if (route === '/products') {
        const navId = navNameToId[normalizedItem] || null;
        dispatch(setProductFilter({ navigationID: navId, collectionID: null, collectionItemID: null }));
      }
      navigate(route);
    } else {
